...
FRED API 返回的 `CPIAUCSL` 是 CPI 指数的绝对值（例如 325.031），而不是通胀率百分比。

脚本会自动计算**同比通胀率（Year-over-Year）**：

```
通胀率 = ((当前月CPI - 12个月前CPI) / 12个月前CPI) × 100%
```

**计算示例**：
- 2024年11月 CPI: 316.45
- 2025年11月 CPI: 325.031
- 通胀率 = ((325.031 - 316.45) / 316.45) × 100% ≈ **2.71%**

### 其他经济指标

- **联邦基金利率 (FEDFUNDS)**: 直接使用 FRED 返回的百分比值
- **失业率 (UNRATE)**: 直接使用 FRED 返回的百分比值

### 数据要求

由于通胀率需要计算同比变化，CPI 数据至少需要 **13 个月的历史数据**才能生成第一个有效的通胀率数据点。

脚本默认获取最近 5 年的数据，确保有足够的历史数据进行计算。
