...
export function ProjectCard({ project, onEdit, selected, onSelect, selectionMode }: Props) {
  ...
  return (
    <div 
      className={`bg-[#1a1a24] rounded-2xl p-5 border transition-all group ${
        selected ? 'border-violet-500 ring-2 ring-violet-500/20' : 'border-white/5 hover:border-white/10'
      }`}
    ...
    >
      ...
      {/* Deadline */}
      {deadline && (
        <div className={`flex items-center gap-1.5 mb-3 text-xs ${deadline.urgent ? 'text-amber-400' : 'text-gray-500'}`}>
      ...
      <div className="flex gap-2 mb-4">
        {project.website && (
          <a
            ...
            target="_blank"
            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-xs text-gray-400 hover:text-white transition-colors"
          >
        ...
        <div className="space-y-1.5 mb-3 max-h-32 overflow-y-auto scrollbar-thin">
          {project.tasks.map((task) => (
            <div key={task.id} className="flex items-center gap-2 group/task">
              ...
              <span className={`flex-1 text-sm ${task.completed ? 'line-through text-gray-600' : 'text-gray-300'}`}>
                {task.title}
              </span>
              <button
                onClick={() => deleteTask(project.id, task.id)}
                className="opacity-0 group-hover/task:opacity-100 p-1 hover:bg-red-500/10 rounded text-gray-600 hover:text-red-400 transition-all"
              >
                <X className="w-3 h-3" />
              </button>
            </div>
      ...
      <form onSubmit={handleAddTask} className="flex gap-2">
        <input
          ...
          placeholder="添加任务..."
          className="flex-1 bg-white/5 border border-white/5 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all placeholder:text-gray-600"
        />
        <button
          type="submit"
          className="px-3 py-2 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm transition-colors"
        >
      ...
      {/* Status Quick Change */}
      <div className="flex gap-1 mt-4 pt-4 border-t border-white/5">
